...
int main() {
    try {
        ...
        std::string onnx_filename = "/home/icv/code/workspaces/inf_onsite/test/onnxfiles/3d_det_all.onnx";
        ...
        profile->setDimensions("input", nvinfer1::OptProfileSelector::kOPT, nvinfer1::Dims4{2, 3, 1080, 1920});  
        profile->setDimensions("input", nvinfer1::OptProfileSelector::kMAX, nvinfer1::Dims4{3, 3, 1080, 1920});  

        config->addOptimizationProfile(profile);
        auto serializedModel = std::unique_ptr<nvinfer1::IHostMemory>(builder->buildSerializedNetwork(*network, *config));

        std::ofstream trtFile("your_model.trt", std::ios::binary);
