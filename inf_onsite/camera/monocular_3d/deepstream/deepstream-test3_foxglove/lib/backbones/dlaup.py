...
class IDAUp(nn.Module):
    ...
    def __init__(self, in_channels_list, up_factors_list, out_channels):
        ...
        self.id = len(in_channels_list) - 2
        for i in range(1, len(in_channels_list)):
            in_channels = in_channels_list[i]
            '''
            specific for each ida upsample
            '''
            up_factors = int(up_factors_list[i])

            if self.id != 0 and up_factors / self.id == 2:
                padding_param = (2, up_factors // 2)
                output_padding_param = (1, 0)
            else:
                padding_param = up_factors // 2
                output_padding_param = 0
            # padding_param = up_factors // 2
            # output_padding_param =0
            self.upfac = up_factors
            ...
            up = nn.ConvTranspose2d(in_channels=out_channels,
                                    ...
                                    output_padding=output_padding_param,
                                    groups=out_channels,
                                    bias=False)
            fill_up_weights(up)
...
class IDAUpv2(nn.Module):
    ...
    def __init__(self, in_channels_list, up_factors_list, out_channels):
        ...
        self.in_channels_list = in_channels_list
        self.out_channels = out_channels
        self.upflist = up_factors_list
        for i in range(1, len(in_channels_list)):
            in_channels = in_channels_list[i]
            up_factors = int(up_factors_list[i])

            print('init decoder', i, up_factors)
            proj = Conv2d(in_channels,
                          out_channels,
                          kernal_szie=3,
                          stride=1,
                          bias=False)
            node = Conv2d(out_channels,
                          out_channels,
                          kernal_szie=3,
                          stride=1,
                          bias=False)
...
class DLAUp(nn.Module):
    def __init__(self, in_channels_list, scales_list=(1, 2, 4, 8, 16)):
        ...
        for i in range(len(in_channels_list) - 1):  # 5
            j = -i - 2
            setattr(
                self, 'ida_{}'.format(i),
                IDAUp(in_channels_list=in_channels_list[j:],
    ...
    def forward(self, layers):
        ...
        for i in range(len(layers) - 1):
            # print(f"ida_{i} -- {len(layers[-i-2:])}")
            ida = getattr(self, 'ida_{}'.format(i))
            # for j in range(len(layers[-i-2:])):
            #     print(layers[j].size())
...
class DLAUpv2(nn.Module):
    def __init__(self, in_channels_list, scales_list=(1, 2, 4, 8, 16)):
        ...
        for i in range(len(in_channels_list) - 1):
            ...
            setattr(
                self, 'ida_{}'.format(i),
                IDAUpv2(in_channels_list=in_channels_list[j:],
                        up_factors_list=scales_list[j:] // scales_list[j],
                        out_channels=in_channels_list[j]))
            scales_list[j + 1:] = scales_list[j]
            in_channels_list[j + 1:] = [
        ...
        self.final_fusion = IDAUpv2(
            in_channels_list=in_channels_list_backup,
            up_factors_list=[
                2**i for i in range(len(in_channels_list_backup))
            ],
