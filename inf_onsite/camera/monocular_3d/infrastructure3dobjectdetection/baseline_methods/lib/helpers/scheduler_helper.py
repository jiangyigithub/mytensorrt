...
def build_lr_scheduler(cfg, optimizer, last_epoch):
    ...
    if cfg['warmup']:
        warmup_lr_scheduler = CosineWarmupLR(optimizer,
                                             num_epoch=5,
                                             init_lr=0.00001)
...
class LinearWarmupLR(lr_sched._LRScheduler):
    ...
    def get_lr(self):
        return [
            self.init_lr +
            (base_lr - self.init_lr) * self.last_epoch / self.num_epoch
            for base_lr in self.base_lrs
        ]
...
if __name__ == '__main__':
    ...
    lr_warmup_scheduler_cosine = CosineWarmupLR(optimizer,
                                                1000,
                                                init_lr=0.00001,
                                                last_epoch=-1)
    lr_warmup_scheduler_linear = LinearWarmupLR(optimizer,
                                                1000,
                                                init_lr=0.00001,
                                                last_epoch=-1)
