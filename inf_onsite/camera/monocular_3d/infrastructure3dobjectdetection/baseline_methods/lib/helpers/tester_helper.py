...
class Tester(object):
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'all':
            ...
            for _, _, files in os.walk(self.cfg['checkpoints_dir']):
                checkpoints_list = [
                    os.path.join(self.cfg['checkpoints_dir'], f) for f in files
                    if f.endswith('.pth')
                ]
    ...
    def inference(self):
        ...
        self.save_results(results,
                          output_dir=self.cfg['save_dir'] + '/outputs')
    ...
    def evaluate(self):
        self.dataloader.dataset.eval(results_dir=self.cfg['save_dir'] +
                                     '/outputs/data',
                                     logger=self.logger)
