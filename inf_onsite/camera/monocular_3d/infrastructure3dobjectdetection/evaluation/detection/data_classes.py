...
class DetectionConfig:
    ...
    def __eq__(self, other):
        ...
        for key in self.serialize().keys():
            eq = eq and \
                np.array_equal(getattr(self, key), getattr(other, key))
        return eq
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(content['class_range'], content['dist_fcn'],
                   content['dist_ths'], content['dist_th_tp'],
                   content['min_recall'], content['min_precision'],
                   content['max_boxes_per_sample'], content['mean_ap_weight'])
...
class DetectionMetricData(MetricData):
    """This class holds accumulated and interpolated data required to calculate
    ...
    def __init__(
        ...
        # Assert ordering.
        assert all(confidence == sorted(
            confidence, reverse=True))  # Confidences should be descending.
...
class DetectionMetrics:
    """Stores average precision and true positive metric results.

    Provides properties to summarize.
    """
    ...
    def add_label_ap(self, detection_name: str, dist_th: float,
                     ap: float) -> None:
    ...
    def mean_ap(self) -> float:
        """Calculates the mean AP by averaging over distance thresholds and
        classes."""
        return float(np.mean(list(self.mean_dist_aps.values())))

    @property
    def tp_errors(self) -> Dict[str, float]:
        """Calculates the mean true positive error across all classes for each
        metric."""
        errors = {}
    ...
    def tp_scores(self) -> Dict[str, float]:
        ...
        for metric_name in TP_METRICS:
            ...
            # Some of the true positive errors are unbounded,
            # so we bound the scores to min 0.
            score = max(0.0, score)
    ...
    def nd_score(self) -> float:
        """Compute the nuScenes detection score (NDS, weighted sum of the
        individual scores).
        ...
        # Normalize.
        total = total / float(self.cfg.mean_ap_weight +
                              len(self.tp_scores.keys()))
...
class DetectionBox(EvalBox):
    """Data class used during detection evaluation.

    Can be a prediction or ground truth.
    """
    def __init__(
        self,
        sample_token: str = '',
        ...
        super().__init__(sample_token, classification, bbox_2d, size,
                         translation, rotation)

        assert classification is not None, \
            'Error: detection_name cannot be empty!'
        assert classification in DETECTION_NAMES, \
            'Error: Unknown detection_name %s' % classification

        # assert attribute_name in ATTRIBUTE_NAMES or attribute_name == '', \
        #     'Error: Unknown attribute_name %s' % attribute_name

        assert type(detection_score
                    ) == float, 'Error: detection_score must be a float!'
        assert not np.any(np.isnan(
            detection_score)), 'Error: detection_score may not be NaN!'
    ...
    def __eq__(self, other):
        return (
            ...
            and self.size == other.size and self.rotation == other.rotation
            # self.velocity == other.velocity and
            # self.ego_translation == other.ego_translation and
            # self.num_pts == other.num_pts and
            # self.detection_name == other.detection_name and
            and self.classification == other.classification and
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(
            ...
            bbox_2d=content['bbox_2d'],
            #    velocity=tuple(content['velocity']),
            #    ego_translation=(0.0, 0.0, 0.0)
            # if 'ego_translation' not in content
            #    else tuple(content['ego_translation']),
            #    num_pts=-1 if 'num_pts' not in content
            # else int(content['num_pts']),
...
class DetectionMetricDataList:
    """This stores a set of MetricData in a dict indexed by (name, match-
    distance)."""
    def __init__(self):
        self.md = {}
    ...
    def get_class_data(
            self,
            detection_name: str) -> List[Tuple[DetectionMetricData, float]]:
        """Get all the MetricData entries for a certain detection_name."""
        return [(md, dist_th) for (name, dist_th), md in self.md.items()
                if name == detection_name]

    def get_dist_data(self,
                      dist_th: float) -> List[Tuple[DetectionMetricData, str]]:
        """Get all the MetricData entries for a certain match_distance."""
        return [(md, detection_name)
                for (detection_name, dist), md in self.md.items()
                if dist == dist_th]

    def set(self, detection_name: str, match_distance: float,
            data: DetectionMetricData):
        """Sets the MetricData entry for a certain detection_name and
        match_distance."""
