...
class EvalBox(abc.ABC):
    ...
    def __init__(
        ...
        #
        #  distance: Tuple[float, float, float] = (0, 0, 0),
        ...
        assert type(
            sample_token) == str, 'Error: sample_token must be a string!'

        # Assert data for shape and NaNs.
        assert type(classification) == str, 'Error: class must be a string!'

        assert len(bbox_2d) == 4, 'Error: Translation must have 3 elements!'
        assert not np.any(
            np.isnan(bbox_2d)), 'Error: Translation may not be NaN!'

        assert len(
            translation) == 3, 'Error: Translation must have 3 elements!'
        assert not np.any(
            np.isnan(translation)), 'Error: Translation may not be NaN!'

        assert len(size) == 3, 'Error: Size must have 3 elements!'
        assert not np.any(np.isnan(size)), 'Error: Size may not be NaN!'

        assert type(rotation) == float, 'Error: Rotation must be float: yaw'
        assert not np.any(
            np.isnan(rotation)), 'Error: Rotation may not be NaN!'
...
class EvalBoxes:
    ...
    def add_boxes(self, sample_token: str, boxes) -> None:
    ...
    def serialize(self) -> dict:
        ...
        return {
            key: [box.serialize() for box in boxes]
    ...
    def deserialize(cls, content: dict, box_cls):
        ...
        for sample_token, boxes in content.items():
            eb.add_boxes(sample_token,
                         [box_cls.deserialize(box) for box in boxes])
        return eb


class MetricData(abc.ABC):
    """Abstract base class for the *MetricData classes specific to each
    task."""
    @abc.abstractmethod
    def serialize(self):
        """Serialize instance into json-friendly format."""
