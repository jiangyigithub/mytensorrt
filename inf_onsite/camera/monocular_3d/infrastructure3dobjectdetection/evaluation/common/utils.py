# Based on nuScenes dev-kit written by Holger Caesar, 2018.
...
def velocity_l2(gt_box: EvalBox, pred_box: EvalBox) -> float:
    ...
    """
    return np.linalg.norm(
        np.array(pred_box.velocity) - np.array(gt_box.velocity))


# def yaw_diff(gt_box: EvalBox, eval_box: EvalBox, period: float = 2*np.pi)\
#     -> float:
#     """
...
def angle_diff(x: float, y: float, period: float) -> float:
    """Get the smallest angle difference between 2 angles: the angle from y to
    x.
...
def attr_acc(gt_box: DetectionBox, pred_box: DetectionBox) -> float:
    """Computes the classification accuracy for the attribute of this class (if
    any). If the GT class has no attributes or the annotation is missing
    attributes, we assign an accuracy of nan, which is ignored later on.

    :param gt_box: GT annotation sample.
    :param pred_box: Predicted sample.
    :return: Attribute classification accuracy (0 or 1)
    or nan if GT annotation does not have any attributes.
    """
    if gt_box.attribute_name == '':
        # If the class does not have attributes or this particular sample is
        # missing attributes, return nan, which is ignored later. Note that
        # about 0.4% of the sample_annotations have no attributes, although
        # they should.
        acc = np.nan
    else:
...
def scale_iou(sample_annotation: EvalBox, sample_result: EvalBox) -> float:
    ...
    It is equivalent to intersection over union (IOU) between the two boxes in
    3D, if we assume that the boxes are aligned, i.e. translation and rotation
...
#     Note that this only works for a quaternion that represents a box in lidar
...
# def boxes_to_sensor(boxes: List[EvalBox], pose_record: Dict,\
# cs_record: Dict):
# """
...
def cummean(x: np.array) -> np.array:
    ...
    if sum(np.isnan(x)) == len(x):
        ...
        sum_vals = np.nancumsum(
            x.astype(float))  # Cumulative sum ignoring nans.
        count_vals = np.cumsum(
            ~np.isnan(x))  # Number of non-nans up to each position.
        return np.divide(sum_vals,
                         count_vals,
