...
void preproc_plausi(MEASLIST_STRUCT *meas_list)
{
  ...
  if (std::abs(meas_list->psiDt_ego) < 0.05) {
    ...
    #if tracker_initial
    std::cout << "0_vx_ego in preproc:"<<vx_ego<<std::endl;
    #endif
  ...
  #if tracker_initial
  std::cout<<"1_v_t_yaw_rate:"<<v_t_yaw_rate<<std::endl;
  #endif
  ...
  for (int idx1_meas = 0; idx1_meas < 511; idx1_meas++) {
    if (meas_list->meas[idx1_meas] == 1.0) {
          ...
          if ((meas_list->meas[i] == 1.0) && (meas_list->dr[i] < 30.0)) {
            if (std::abs(meas_list->dr[idx1_meas] - 0.5 * meas_list->dr[i]) <
              ...
              if (std::abs(meas_list->vr[idx1_meas] - 0.5 * meas_list->vr[i]) <
                ...
                #if tracker_initial
                auto temp_01 = std::abs(v_t_yaw_rate / static_cast<double>(trueCount) -
                             sens_mount_angle / static_cast<double>(b_trueCount));
                std::cout<<"0_potDoubleRelf_condi:"<<temp_01<<std::endl;
                #endif
              }
              ...
              #if tracker_initial
                auto temp_02 = std::abs(v_t_yaw_rate / static_cast<double>(trueCount) -
                           sens_mount_angle / static_cast<double>(b_trueCount));
                std::cout<<"1_potDoubleRelf_condi_02:"<<temp_02<<std::endl;
              #endif
              if (std::abs(v_t_yaw_rate / static_cast<double>(trueCount) -
                           sens_mount_angle / static_cast<double>(b_trueCount)) <
                  0.17453292519943295) {
      ...
      #if tracker_initial
      std::cout<<"2_get into location behind/ front judge"<<std::endl;
      #endif
      ...
      if (std::abs(meas_list->vr[idx1_meas]) < 0.02) {
       ...
       #if tracker_initial
       std::cout<<"2_dBRcs:"<<meas_list->dBRcs[idx1_meas]<<std::endl;
       #endif
