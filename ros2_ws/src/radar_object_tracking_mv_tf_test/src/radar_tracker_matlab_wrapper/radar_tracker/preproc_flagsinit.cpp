  ...
  meas_list_fov_angle[25], double dt)
{
  ...
  // std::cout<<"00_getinto preproc:"<<obj_px_no_offset<<"value:"<<meas_list_fov_angle[0]<<"and:"<<meas_list_fov_angle[24]<<std::endl; //lnl

    //lnl, CANFD no FOV issue
  // if (sizeof(meas_list_fov_angle))

  // for(int i=0;i<=25;i++)
  // {
  //   std::cout<<"00_fov_angle:"<<meas_list_fov_angle[i]<<"01_fov_range"<<meas_list_fov_range[i]<<std::endl;

  // }
  if ((meas_list_fov_angle[0]==0)||(meas_list_fov_range[0]==0))
  {
    obj->measureable = true;
  }

  //lnl, used to be if
  else if ((obj_px_no_offset < meas_list_fov_angle[0]) || (obj_px_no_offset >
       meas_list_fov_angle[24])) {  //lnl
  // if ((obj_px_no_offset < 60) || (obj_px_no_offset >
  //      (-60))) {
    //  Object out of FOV bounds
    obj->measureable = false;
  } else {
    
    std::cout<<"00_normal:"<<std::endl; //lnl
    int idx;
    int ii_size_idx_1;
    int ii;
    bool exitg1;
