                 ...
                 *is_associated, bool *mark_as_associated)
{
  ...
  *mark_as_associated = false;
  ...
  // }
  //lnl, test
  range_gate_abs = y[0] / 40.0;
  if (!(range_gate_abs > 2.0)) {
    range_gate_abs = 2.0;
  }
  ...
  //lnl, test
  range = std::sqrt(obj->x[0] * obj->x[0] + obj->x[3] * obj->x[3]) / 12.0;
  v_radial_max = std::abs(y[2] * 0.15);
                 ...
                 *is_associated, bool *mark_as_associated)
{
  ...
  *mark_as_associated = false;
  ...
  //lnl, test, new gating
  double proj_x_threshold;
  double proj_y_threshold;
  double proj_x;
  double proj_y;
  // double range_gate_abs_x;
  // double range_gate_abs_y;
  // double angle_gate_abs;
  // double vr_absGate_x;  
  // double vr_absGate_y; 
  // double range_max_x; 
  ...
  // lnl, test
  proj_x = obj->length*std::abs(std::cos(obj->psi))+obj->width*std::abs(std::sin(obj->psi));
  proj_y = obj->length*std::abs(std::sin(obj->psi))+obj->width*std::abs(std::cos(obj->psi));
  // proj_x_threshold = 5*std::abs(std::cos(obj->psi)) + 2*std::abs(std::sin(obj->psi));  //4
  // proj_y_threshold = 5*std::abs(std::sin(obj->psi)) + 2*std::abs(std::cos(obj->psi));  //4
  if ((obj->length * obj->width) < 15.0)
  {
      proj_x_threshold = 5*std::abs(std::cos(obj->psi)) + 2.5*std::abs(std::sin(obj->psi));  //4
      proj_y_threshold = 5*std::abs(std::sin(obj->psi)) + 2.5*std::abs(std::cos(obj->psi));  //4
  }
  else
  {
    proj_x_threshold = 5*std::abs(std::cos(obj->psi)) + 2.5*std::abs(std::sin(obj->psi));  //4
  ...
  // }
  //lnl, test
  range_gate_abs = std::max((double)(y[0]/20),(double)1)*std::max((double)(proj_x_threshold - proj_x), (double)0)/2 + 0.5 ;//y[0] / 40.0;
  // if (!(range_gate_abs > 2.0)) {
  //   range_gate_abs = 5;//2.0;
  // }


  //lnl, test
  // angle_gate_abs = 0.034906585039886591;  //2 degree

  angle_gate_abs = atan2((std::max((double)(proj_y_threshold - proj_y), (double)0)/2+0.5),y[0]); //0.0174533;

  #if tracker_initial
    std::cout<<" 0011_proj_x: "<<proj_x<<std::endl;
    std::cout<<" 0011_proj_y: "<<proj_y<<std::endl;
    std::cout<<" 0011_proj_length: "<<obj->length<<std::endl;
    std::cout<<" 0011_proj_width: "<<obj->width<<std::endl;
    std::cout<<" 0011_proj_x_threshold: "<<proj_x_threshold<<std::endl;
    std::cout<<" 0011_proj_y_threshold: "<<proj_y_threshold<<std::endl;
    std::cout<<" 0011_y[0]: "<<y[0]<<std::endl;   
    std::cout<<" 0011_range_gate_abs: "<<range_gate_abs<<std::endl;
    std::cout<<" 0011_angle_gate_abs: "<<angle_gate_abs<<std::endl;
  #endif
  ...
  // //lnl, test
  // range = std::sqrt(obj->x[0] * obj->x[0] + obj->x[3] * obj->x[3]) / 1.0;
  // v_radial_max = std::abs(y[2] * 1);
  ...
  //   //  ignoring elevation makes a difference for near objects
  //   vr_absGate = (vr_absGate + 0.5 * std::abs(vx_ego)) + 0.2 * std::abs(y[2]);
  //   angle_gate_abs = 0.0035;//(17.0 - y[0]) * 3.1415926535897931 / 180.0;
  ...
  //----lnl,tracker initial
  auto angle_ass_gate = std::abs(atan2(2,y[0]));  
  for (range = y[1] - obj_angle; range < -3.1415926535897931; range +=
    6.2831853071795862) {
  }
  ...
  if ((y[0] < range_max) && (y[0] > range_min)) {
    ...
    #if tracker_initial
    std::cout<<"0_ass_gating_ass_marker---"<<"y[0]:"<<y[0]<<" range_max:"<<range_max<<" range_min:"<<range_min<<" range:"<<range<<" b_py_sensor:"<<b_py_sensor<<" angle_max:"<<angle_max<<"y[2]:"<<y[2]<<
    "py_sensor:"<<py_sensor<<" v_radial_max:"<<v_radial_max<<" vr_absGate:"<<vr_absGate<<std::endl;
    #endif

 

    if ((range > b_py_sensor) && (range < angle_max)) {   //lnl, tracker initial, because this condition not fulfilled, b_py_sensor is angle_min
      //  vr-Gating
      //  velocity differs for parts of car, when vehicle is nearby (e.g. crossing in front of us) 
      if ((y[2] > py_sensor) && (y[2] < v_radial_max)) {
        y_aso[0] = y[0];
    ...
    else if ((range > b_py_sensor-angle_ass_gate) && (range < angle_max+angle_ass_gate)) //lnl
    {
      *mark_as_associated = true;
      *is_associated = false;
    }
    ...
    #if tracker_initial
    std::cout<<"1_ass_gating---"<<"is_assiciated:"<<*is_associated<<"mark_as_associated:"<<*mark_as_associated<<std::endl;
    #endif
  }
  else if ((y[0] < range_max+range_gate_abs+2) && (y[0] > range_min-range_gate_abs-2))
  {

    // auto angle_ass_gate = std::abs(atan2(1,y[0])); 

    #if tracker_initial
    std::cout<<" 2_angle_ass_gate: "<<angle_ass_gate<<" range_gate_abs: "<<range_gate_abs<<std::endl;
    #endif
    // for (range = y[1] - obj_angle; range < -3.1415926535897931; range +=
    //      6.2831853071795862) {
    // }
    ...
    //   range -= 6.2831853071795862;
    // }
    if ((range > b_py_sensor - angle_ass_gate) && (range < angle_max + angle_ass_gate)){
      *mark_as_associated = true;
      *is_associated = false;
    }
    #if tracker_initial
  std::cout<<"99_after_ass_gating---"<<"is_assiciated:"<<*is_associated<<"mark_as_associated:"<<*mark_as_associated<<" range: "<<range<<" y[0]: "<<y[0]<<std::endl;
  #endif
  }
//   #if tracker_initial
//   std::cout<<"99_after_ass_gating---"<<"is_assiciated:"<<*is_associated<<"mark_as_associated:"<<*mark_as_associated<<" range: "<<range<<" y[0]: "<<y[0]<<std::endl;
//   #endif
    ...
    #if tracker_initial
    std::cout<<" 999_ass_gating_ass_marker--- "<<"y[0]:"<<y[0]<<" range_max:"<<range_max<<" range_min:"<<range_min<<" range:"<<range<<" b_py_sensor:"<<b_py_sensor<<" angle_max:"<<angle_max<<" y[2]:"<<y[2]<<
    " py_sensor:"<<py_sensor<<" v_radial_max:"<<v_radial_max<<" vr_absGate:"<<vr_absGate<<std::endl;
    #endif
  }
...
//
//
// lnl, trial of association in X-O-Y coor instead of  polar coor
//
// [EOF]
//

void asso_gating_xoy(OBJECT_STRUCT *obj, const double y[3], const double
                 sensor_pos_offset[2], double vx_ego, double y_aso[3], bool
                 *is_associated, bool *mark_as_associated)
{
  double obj_angle;
  ...
  *mark_as_associated = false;
  ...
  double sym_loc2obj;
  double proj_deg_loc2obj;

  double distance_x;
  double distance_y;

  double proj_x_dis2obj;
  double proj_y_dis2obj;
  ...
  measurement_x = y[0] * std::cos(y[1]);
  measurement_y = y[0] * std::sin(y[1]);

  sym_loc2obj = obj->x[1]*obj->x[4];
  

  distance_x = measurement_x - obj->x[0];
  distance_y = measurement_y - obj->x[3];

  if(sym_loc2obj < 0)
  {
      proj_deg_loc2obj = std::atan(std::abs(std::tan(obj->psi))) + rt_atan2d_snf(std::abs(distance_y),std::abs(distance_x));
  }
  else
  {
      proj_deg_loc2obj = std::abs(std::atan(std::abs(std::tan(obj->psi))) - rt_atan2d_snf(std::abs(distance_y),std::abs(distance_x))); 
  }

  proj_x_dis2obj = std::sqrt(std::pow(distance_x,2) + std::pow(distance_y,2))*std::cos(proj_deg_loc2obj);
  proj_y_dis2obj = std::sqrt(std::pow(distance_x,2) + std::pow(distance_y,2))*std::sin(proj_deg_loc2obj);


  range_gate_x =(5 - obj->length)/2 + 0.5;
  range_gate_y = (2.5 - obj->width)/2;

  range_max_x = (obj->length)/2  + std::max((double)range_gate_x,(double)0) + 0.5;
  range_min_x = -range_max_x;

  range_max_y = (obj->width)/2 + std::max((double)range_gate_y, (double)0) + 1;
  ...
  //  velocity interval   //lnl, corner_range[3] is actually doing the same calculation, no need to write in different way
  corner_ranges[0] = measurement1_idx_2;
  corner_ranges[1] = measurement2_idx_2;
  corner_ranges[2] = measurement3_idx_2;
  corner_ranges[3] = ((state_augmented_idx_1 - vx_ego) * b_px_sensor +
  ...
  //lnl, new gating condition judgement
  if ((proj_x_dis2obj < range_max_x) && (proj_x_dis2obj > range_min_x) && (proj_y_dis2obj < range_max_y) && (proj_y_dis2obj > range_min_y))
  {
    
    if ((std::abs(proj_x_dis2obj) < 5) && std::abs(proj_y_dis2obj) < 2)
    {
      if ((y[2] > py_sensor) && (y[2] < v_radial_max)) {
        y_aso[0] = y[0];
        y_aso[1] = y[1];
    ...
    else
    {
      ...
      //     *mark_as_associated = true;
      // }   
      else if ((y[2] > 2*py_sensor) && (y[2] < 2*v_radial_max))
      {
          *is_associated = false;
          *mark_as_associated = true;
      }
  ...
  else if ((proj_x_dis2obj < range_max_x + 0.5) && (proj_x_dis2obj > range_min_x - 0.5) && (proj_y_dis2obj < range_max_y + 0.2) && (proj_y_dis2obj > range_min_y - 0.2))
  {
      *mark_as_associated = true;
      *is_associated = false;   
  }
