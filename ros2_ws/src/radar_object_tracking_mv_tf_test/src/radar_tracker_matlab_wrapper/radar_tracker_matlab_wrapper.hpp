...
const std::chrono::milliseconds duration_1ms (1);
...
public:
  ...
  double getObjtestdelta(std::size_t idx) const {return m_obj_list_ast[idx].test_delta;}//lnl, delta test only
  double getObjmoving(std::size_t idx) const {return m_obj_list_ast[idx].moving;}
  double getObjPgr_reflex(std::size_t idx) const {return m_obj_list_ast[idx].Pgr_reflex;}
  double getObjage(std::size_t idx) const {return m_obj_list_ast[idx].age;}
  double getObjRCS_filt(std::size_t idx) const {return m_obj_list_ast[idx].RCS_filt;}
