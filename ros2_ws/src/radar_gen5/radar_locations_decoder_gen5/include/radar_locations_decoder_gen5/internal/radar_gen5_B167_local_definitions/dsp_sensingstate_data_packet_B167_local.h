...
//PRQA S 1025 EOF
// 1025: Macro may be used as a literal.
...
// --> there is no problem, because the struct name starts
//          with a module name from the DSP component
// PRQA S 2400 1
...
struct DSP_MISALIGNMENT_ST           // PRQA S 2400
{

public:
   // Constructor
   DSP_MISALIGNMENT_ST() : thetaMalAng(0.0F), thetaMalAngVar(0.0F),
         phiMalAng(0.0F), phiMalAngVar(0.0F), malStatus(0U), malStatusEme(0U), phiMalAngEme(0.0F), phiMalAngEmeVar(0.0F)
   { };
...
enum ValidFlgState              // PRQA S 2400
{
...
struct DSP_BLINDNESS_ST         // PRQA S 2400
{
...
struct DSP_INTERFERENCE_ST                 // PRQA S 2400
{
...
struct DSP_FIELDOFVIEW_ST                  // PRQA S 2400
{
...
struct DSP_SENSINGSTATE_DATA_PACKET_ST              // PRQA S 2400
{
...
public:
   ...
   */
   vfc::float32_t getViewDistance(vfc::float32_t angleAzimuth, vfc::float32_t angleElevation) const
   {
      ...
      else if (aziDeg >(vfc::float32_t) FieldOfView_st.thetaViewAry[MAXNUMTHETAFOV - 1])
      {
         dMaxAziFoV = 0.0f;
      }
      else
      ...
      else if (elevDeg >(vfc::float32_t) FieldOfView_st.phiViewAry[MAXNUMPHIFOV - 1])
      {
         facSDMaxElevFoV = 0.0f;
      }
      else
