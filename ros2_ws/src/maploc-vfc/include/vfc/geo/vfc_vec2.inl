...
vfc::TVector2<T>::TVector2    (const TVector2<U>& rhs)                                        
{    
...
const vfc::TVector2<T>&    vfc::TVector2<T>::operator*= (const T& rhs)
{
    m_data[0] *= rhs;
    m_data[1] *= rhs;
...
template <class T>    inline
const vfc::TVector2<T>&    vfc::TVector2<T>::divide_assign(const T& rhs, true_t)
{
...
template <class T>    inline
const vfc::TVector2<T>&    vfc::TVector2<T>::divide_assign (const T& rhs, false_t)
{
    m_data[0] /= rhs;
    m_data[1] /= rhs;
...
template <class T>    inline
const vfc::TVector2<T>&    vfc::TVector2<T>::operator+= (const TVector2<T>& rhs)
{
...
template <class T>    inline
void    vfc::TVector2<T>::set    (const T& val1, const T& val2)    
{    
    m_data[0] = val1; 
    m_data[1] = val2; 
}
...
template <class T>    inline
void    vfc::TVector2<T>::set    (const T& val)                                    
{    
    m_data[0] = m_data[1] = val;
}

template <class T>    inline
const    T&    vfc::TVector2<T>::operator[](int32_t idx)    const    
{    
    VFC_ASSERT(0<=idx && 1>=idx); 
    return m_data[idx];
}

template <class T>    inline
T&    vfc::TVector2<T>::operator[](int32_t idx)            
{    
    VFC_ASSERT(0<=idx && 1>=idx); 
    return m_data[idx];
}
...
Revision 1.3 2006/01/11 11:37:53CET Zitzewitz Henning von (CR/AEM5) * (ZVH2HI) 
