...
#ifndef    VFC_ASSERT_MAT3
#    define    VFC_ASSERT_MAT3(col,row) VFC_ASSERT2( (0<=(col) && 0<=(row) && 2>=(col) && 2>= (row)),"mat3 idx out of bounds")
#endif
...
vfc::TMatrix3<T>::TMatrix3(const TMatrix3<U>& rhs)
{
    set    (    static_cast<T>(rhs(0,0)), static_cast<T>(rhs(0,1)), static_cast<T>(rhs(0,2)),
            static_cast<T>(rhs(1,0)), static_cast<T>(rhs(1,1)), static_cast<T>(rhs(1,2)),
            static_cast<T>(rhs(2,0)), static_cast<T>(rhs(2,1)), static_cast<T>(rhs(2,2)));
}

template <class T> inline
vfc::TMatrix3<T>::TMatrix3
...
template <class T> inline
void    vfc::TMatrix3<T>::set (    const T& val)
{
    set    (    val, val, val,
            val, val, val,
            val, val, val);
...
template <class T>    inline
const    T&    vfc::TMatrix3<T>::operator()(int32_t row, int32_t col)    const
{
    VFC_ASSERT_MAT3(col,row);
    return m_data[row][col];
}

template <class T>    inline
T&    vfc::TMatrix3<T>::operator()(int32_t row, int32_t col)
{
    VFC_ASSERT_MAT3(col,row);
    return m_data[row][col];
}
...
template <class T>    inline
const vfc::TMatrix3<T>&    vfc::TMatrix3<T>::operator*= (const T& rhs)
{
...
template <class T>    inline
const vfc::TMatrix3<T>&    vfc::TMatrix3<T>::divide_assign(const T& rhs, true_t)
{
...
template <class T>    inline
const vfc::TMatrix3<T>&    vfc::TMatrix3<T>::divide_assign(const T& rhs, false_t)
{
    m_data[0][0] /= rhs; m_data[0][1] /= rhs; m_data[0][2] /= rhs;
    m_data[1][0] /= rhs; m_data[1][1] /= rhs; m_data[1][2] /= rhs;
    m_data[2][0] /= rhs; m_data[2][1] /= rhs; m_data[2][2] /= rhs;
...
template <class T>    inline
const vfc::TMatrix3<T>&    vfc::TMatrix3<T>::operator+= (const TMatrix3<T>& rhs)
{
...
|-------------------------------------------------------------------|
$Log: vfc_mat3.inl  $
Revision 1.6 2009/02/02 05:53:55MEZ gaj2kor 
-Removal of QAC++ warnings.
(Mantis : 0002494)
Revision 1.5 2007/03/16 16:05:07IST Muehlmann Karsten (CC-DA/ESV2) (muk2lr) 
